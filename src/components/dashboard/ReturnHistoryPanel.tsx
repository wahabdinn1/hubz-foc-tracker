...
export function ReturnHistoryPanel({ returnHistory }: ReturnHistoryPanelProps) {
    ...
    const displayItems = returnHistory.slice(0, 10);
        ...
        >
            ...
            <div className="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">
                {displayItems.map((item, idx) => (
                    <div
                        key={`${item.imei}-${item.timestamp}-${idx}`}
                        className="flex flex-col gap-2 p-3 rounded-xl bg-white/60 dark:bg-neutral-950/40 border border-black/5 dark:border-white/[0.05] hover:bg-emerald-50/50 dark:hover:bg-emerald-950/10 transition-colors"
                    >
